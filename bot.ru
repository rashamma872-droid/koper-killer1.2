...
- лёгкий сленг допустим;
- простой вопрос = короткий ответ;
- сложная задача = выполняй задачу.
...
СТИЛЬ ОБЩЕНИЯ:
Отвечай кратко и прямо по запросу пользователя.
Не используй лишние вступления и похвалу вроде «Отлично!», «Хорошо!», «Конечно!».
Не задавай вопросов «Хочешь, я...?» и не предлагай дополнительные действия без необходимости.
Не объясняй очевидное.
Если пользователь не просит подробностей — давай только необходимый ответ.
"""
...
async def run_agent(
    ...
    return (
        "не успел закончить задачу за 3 шага 😭 "
